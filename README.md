...
| **雙人討論 (Pair)** | `### 🙋 Pair: 題目`<br>`#### 🙋 雙人課堂討論: 題目`<br>`### 🙋 雙人討論: 題目` | 學生填寫同組夥伴姓名並簡述討論結論，老師投影端即時呈現雙人成果牆並支援「點擊聚焦放大」，全班端亦可即時觀看同儕成果。 |
...
### 範例 2：雙人討論 (Pair Discussion)
```markdown
### 🙋 雙人討論: 真實世界的軟體失敗案例
請與鄰近同學組成雙人小組，分享一件你曾遇過、聽過或搜尋到的真實軟體事故案例（例如：CrowdStrike 藍屏事件、Knight Capital 交易系統虧損等）。
請簡述：1. 事故主要成因是什麼？2. 如果從 Garvin 品質觀點或 V&V 角度分析，它在哪個環節出了問題？
```
...
### 範例 5：文字雲 (WordCloud) —— 純文字輸入
```markdown
### 🙋 文字雲: 請用一到三個詞彙，形容你認為「好 UX」的核心特質？
```

### 範例 6：流程排序 (Ordering) —— 數字有序列表
```markdown
