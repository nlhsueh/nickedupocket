...
## Ch 1: 軟體危機、品質模型與 AI 時代的可靠性工程

### [Activity: sqa-ch01-ccq1] 愛國者飛彈攔截失效原因 (CCQ 1)
...
### [Activity: sqa-ch01-pair1] 軟體事故與失效案例分析 (Pair 2)
...
### [Activity: sqa-ch01-ccq2] AI 程式碼流失率 Code Churn (CCQ 3)
...
### [Activity: sqa-ch01-wordcloud1] 最重要品質指標關鍵字 (WordCloud 4)
#### [WordCloud] 你覺得哪一個觀點是最重要的品質指標？請寫下來。

### [Activity: sqa-ch01-ccq3] Garvin 品質觀點判斷 (CCQ 5)
...
### [Activity: sqa-ch01-ccq4] Verification vs Validation 辨析 (CCQ 6)
...
### [Activity: sqa-ch01-ordering1] V-Model 生命週期順序排序 (Ordering 7)
...
## Ch 2: 錯與除錯 (Bugs, Faults, and Debugging)

### [Activity: sqa-ch02-ccq1] 轉帳手續費缺陷與失效 (CCQ 1)
...
### [Activity: sqa-ch02-ccq2] 負數年齡輸入與規格遺漏 (CCQ 2)
...
### [Activity: sqa-ch02-ccq3] 巢狀判斷與 Clean Code 重構 (CCQ 3)
...
### [Activity: sqa-ch02-ccq4] 內部品質 vs 外部品質 (CCQ 4)
...
### [Activity: sqa-ch02-ccq5] 吞掉例外 Swallowing Exception (CCQ 5)
...
## Ch 3: 軟體測試原則、理論與架構模型 (AI 時代前沿版)

### [Activity: sqa-ch03-ccq1] 契約式設計 Design by Contract (CCQ 1)
...
### [Activity: sqa-ch03-ccq2] AI 測試與殺蟲劑悖論 (CCQ 2)
...
### [Activity: sqa-ch03-ccq3] V-Model 高階架構對應測試 (CCQ 3)
...
### [Activity: sqa-ch04-ccq1] 靜態測試早期發現錯誤 (CCQ 1)
#### [CCQ] 靜態測試（如軟體檢視、規格檢視）可以在程式碼實際執行之前，檢查需求、設計、程式碼甚至測試資料中的異常，以早期發現錯誤、降低整體的軟體品質成本。
- 正確 (True) (Correct)
- 錯誤 (False)

### [Activity: sqa-ch04-ccq2] Fagan 軟體檢視概述階段 (CCQ 2)
...
### [Activity: sqa-ch04-ccq3] 規格雙向追溯性 (CCQ 3)
...
### [Activity: sqa-ch04-ccq4] 設計檢視執行時機 (CCQ 4)
#### [CCQ] 設計檢視（Design Review）最理想的執行時機，是在系統所有模組的單元測試與整合測試皆通過之後，以確保實際產出的系統與設計文件相符。
- 正確 (True)
- 錯誤 (False) (Correct)

### [Activity: sqa-ch04-ccq5] OWASP 例外資訊洩漏漏洞 (CCQ 5)
...
### [Activity: sqa-ch04-ccq6] 檢視速率與品質度量 (CCQ 6)
...
### [Activity: sqa-ch05-ccq1] JUnit assertSame 與 assertEquals (CCQ 1)
#### [CCQ] 在 JUnit 中，`assertSame(a, b)` 斷言的作用與 `assertEquals(a, b)` 完全相同，都是在驗證兩個物件的內容值是否相等（即比對 `a.equals(b)`）。

### [Activity: sqa-ch05-ccq2] 獨立型強固邊界測試 BVA (CCQ 2)
...
### [Activity: sqa-ch05-ccq3] 等價分割弱與強分類法 (CCQ 3)
...
### [Activity: sqa-ch05-ccq4] 全成對測試 Pairwise 原理 (CCQ 4)
...
### [Activity: sqa-ch05-ccq5] 正交表與成對測試比較 (CCQ 5)
...
### [Activity: sqa-ch05-ccq6] 決策表測試 Decision Table (CCQ 6)
...
### [Activity: sqa-ch05-ccq7] 狀態覆蓋與轉移覆蓋強度 (CCQ 7)
...
### [Activity: sqa-ch05-ccq8] 屬性基礎測試 Property-Based (CCQ 8)
...
### [Activity: sqa-ch06-ccq1] JaCoCo 覆蓋率測量原理 (CCQ 1)
#### [CCQ] JaCoCo 作為 Java 軟體測試覆蓋率工具，在測量覆蓋率時是透過修改 Java 原始碼檔 (.java) 並插入計數器變數來追蹤執行狀態的。
- 正確 (True)
- 錯誤 (False) (Correct)

### [Activity: sqa-ch06-ccq2] 短路求值與分支/條件涵蓋 (CCQ 2)
...
### [Activity: sqa-ch06-ccq3] MC/DC 覆蓋率測試案例數 (CCQ 3)
...
### [Activity: sqa-ch06-ccq4] 圈複雜度與基本路徑測試 (CCQ 4)
...
### [Activity: sqa-ch06-ccq5] 變異測試與等價變異體 (CCQ 5)
...
### [Activity: sqa-ch06-ccq6] LLM 輔助單元測試斷言審查 (CCQ 6)
...
### [Activity: sqa-ch08-ccq1] Cucumber Scenario 與 Background (CCQ 1)
